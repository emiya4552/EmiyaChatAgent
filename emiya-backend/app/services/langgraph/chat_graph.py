...
仅含 6 个节点的"分析流水线"。回复生成由 `chat_service` 直接调
...
def build_analysis_graph() -> StateGraph:
    ...
    workflow.add_node("prepare_history", node_prepare_history)
    workflow.add_node("build_prompt", node_build_prompt)

    # 检索记忆 -> 激活世界书 -> 解析人设片段 -> 好感度评估 -> 准备历史窗口 -> 构建 prompt
    ...
    workflow.add_edge("resolve_profile_section", "assess_relationship")
    workflow.add_edge("assess_relationship", "prepare_history")
    workflow.add_edge("prepare_history", "build_prompt")
